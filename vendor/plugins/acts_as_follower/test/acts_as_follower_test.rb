...
class ActsAsFollowerTest < Test::Unit::TestCase
  ...
  context "acts_as_follower" do
    ...
    context "following" do
      ...
      should "not return following when the follow is inactive" do
        @jon.follow(@sam, :active => false)
    ...
    context "follow a friend and set it as inactive" do
      setup do
        @jon.follow(@sam, :active => false)
      end
    ...
    context "follows" do
      ...
      context "follows_by_type" do
        ...
        should "not return inactive follows" do
          @jon = Factory(:jon)
          @metallica = Factory(:metallica)

          @jon.follow(@metallica, :active => false)

          @band_follow = Follow.find(:first, :conditions => ["follower_id = ? and follower_type = 'User' and followable_id = ? and followable_type = 'Band'", @jon.id, @metallica.id])

          assert_not_equal [@band_follow], @jon.follows_by_type('Band')
        end
      end
    ...
    context "following_by_type" do
      ...
      should "not return inactive follows" do
        ...
        assert_equal [], @jon.following_by_type('User')
      end
    end
